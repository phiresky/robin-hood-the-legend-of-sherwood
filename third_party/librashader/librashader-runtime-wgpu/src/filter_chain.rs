...
use librashader_reflect::reflect::semantics::{ShaderSemantics, TextureSemantics};
...
impl FilterChainWgpu {
    ...
    pub fn load_from_pack_deferred(
        ...
        let feedback_enabled = Self::init_feedback_mask(&filters);
    ...
    fn init_feedback_mask(filters: &[FilterPass]) -> Box<[bool]> {
        let mut feedback_enabled = vec![false; filters.len()];

        // RetroArch only allocates and rotates feedback framebuffers for passes that are
        // actually referenced as PassFeedbackN. Rotating every pass can change multipass
        // feedback semantics for presets with temporal averaging.
        for pass in filters {
            for semantic in pass.reflection.meta.texture_meta.keys() {
                if semantic.semantics == TextureSemantics::PassFeedback
                    && semantic.index < feedback_enabled.len()
                {
                    feedback_enabled[semantic.index] = true;
                }
            }

            for semantic in pass.reflection.meta.texture_size_meta.keys() {
                if semantic.semantics == TextureSemantics::PassFeedback
                    && semantic.index < feedback_enabled.len()
                {
                    feedback_enabled[semantic.index] = true;
        ...
        feedback_enabled.into_boxed_slice()
    }
    ...
    pub fn frame<'a>(
        ...
        // swap output and feedback **before** recording command buffers. Match RetroArch:
        // only passes that are referenced as PassFeedbackN rotate their feedback buffer.
        for (index, enabled) in self.feedback_enabled.iter().copied().enumerate() {
            if enabled {
                std::mem::swap(
        ...
        OwnedImage::scale_framebuffers_with_context(
            ...
            Some(&mut |index: usize,
                       ...
                       feedback: &OwnedImage| {
                // refresh inputs
                self.common.feedback_textures[index] = self.feedback_enabled[index]
                    .then(|| feedback.as_input(pass.meta.filter, pass.meta.wrap_mode));
        ...
        if self.require_clear {
            for history in &self.history_framebuffers {
                history.clear(cmd);
            }
            for (enabled, feedback) in self
                .feedback_enabled
