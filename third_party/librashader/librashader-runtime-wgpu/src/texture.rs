...
impl OwnedImage {
    ...
    pub fn scale(
        ...
        let max_miplevels = if mipmap {
            size.calculate_miplevels()
        } else {
            1
        };

        if self.size != size || self.max_miplevels != max_miplevels || format != self.image.format()
        {
            let mut new = OwnedImage::new(device, size, max_miplevels, format.into());
    ...
    pub fn clear(&self, cmd: &mut wgpu::CommandEncoder) {
        let _pass = cmd.begin_render_pass(&wgpu::RenderPassDescriptor {
            label: Some("librashader clear framebuffer"),
